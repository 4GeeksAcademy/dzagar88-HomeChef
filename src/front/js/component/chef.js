...
export const Chef = () => {
    ...
    const [addButtonLabel, setAddButtonLabel] = useState("Add"); // Added state for button label
    ...
    const toggleMenuItemForm = () => {
        ...
        setAddButtonLabel(showMenuItemForm ? "Add" : "Cancel"); // Toggle button label
    };
    ...
    return (
        ...
        >
            ...
            <div className="container">
                ...
                >
                    Click "{addButtonLabel}" to {showMenuItemForm ? "cancel adding" : "add"} a menu item:
                </div>
                <div className="d-flex justify-content-center">
                    <div>
                        <button
                            className="container btn white-title oy-button my-2 shadow1 title-font"
                            onClick={toggleMenuItemForm}
                        >
                            {addButtonLabel}
                        </button>
            ...
            {store.menuItems.map(item => (
                <div key={item.id}>
                    <div
                        key={item.id}
                        className="container card title-font border border-dark my-3"
                        style={{ width: "20rem" }}
                    >
                        <h3>{item.title}</h3>
